...
export default function AsideMenu() {
  ...
  // Функция-обработчик клика с асинхронным запросом и обработкой ошибок
  const handleMenuClick = async (endpointName: AsideEndpointsUnion) => {
    if (isLoading) return; // Блокируем повторные клики при загрузке

    try {
      dispatch(setSelectedServer(endpointName)); // Сразу меняем UI
  ...
  return (
    <nav className={styles.nav}>
      {[
        { key: "CS:GO", label: "🎮 CS GO" }, // ← ДОБАВЛЕНО СЮДА — ПЕРВЫМ!
        { key: "CS2", label: "🎮 CS2" },
        { key: "add-server", label: "➕ Добавить сервер" },
        { key: "premium", label: "💎 Раскрутка сервера" },
        ...
        <div
          ...
          onClick={() => handleMenuClick(item.key as AsideEndpointsUnion)}
        >
          {item.label}
